...
"""
from airflow.models import DAG, DagRun, TaskInstance, Log, XCom, SlaMiss, DagModel, Variable, TaskReschedule
...
DEFAULT_MAX_DB_ENTRY_AGE_IN_DAYS = int(Variable.get("max_db_entry_age_in_days",
                                                    5))  # Length to retain the log files if not already provided in the conf. If this is set to 30, the job will remove those files that are 30 days old or older.
...
DATABASE_OBJECTS = [  # List of all the objects that will be deleted. Comment out the DB objects you want to skip.
    {
        "airflow_db_model": DagRun, "age_check_column": DagRun.execution_date, "dag_id": DagRun.dag_id,
        "keep_last_run":    True
    },
    {
        "airflow_db_model": TaskReschedule, "age_check_column": TaskReschedule.execution_date,
        "dag_id":           TaskReschedule.dag_id, "keep_last_run": False
    },
    {
        "airflow_db_model": TaskInstance, "age_check_column": TaskInstance.execution_date,
        "dag_id":           TaskInstance.dag_id, "keep_last_run": False
    },
    {
    ...
    {
        "airflow_db_model": BaseJob, "age_check_column": BaseJob.latest_heartbeat, "dag_id": BaseJob.dag_id,
        "keep_last_run":    False
    },
    {
        "airflow_db_model": SlaMiss, "age_check_column": SlaMiss.execution_date, "dag_id": SlaMiss.dag_id,
        "keep_last_run":    False
    },
    {
        "airflow_db_model": DagModel, "age_check_column": DagModel.last_scheduler_run, "dag_id": DagModel.dag_id,
        "keep_last_run":    False
    },
]
...
def cleanup_function(**context):
    ...
    if keep_last_run:
        ...
        query = query.filter(age_check_column.notin_(sub_query), and_(age_check_column <= max_date))
    else:
